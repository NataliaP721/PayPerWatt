...
void loop() {
  ...
  //R = 220, but 270 gave better experimental results if used in the code instead?
  double RMSCurrent = ((Vpp*4.9/1000)-2.5137)/270/2*0.707*2000; 
  // if the current is below 0, set the measurement to 0. 
  if(RMSCurrent<0){
    RMSCurrent = 0;
  }
  
  int RMSPower = 110*2/3*RMSCurrent;  // Calculates RMS Power Assuming Voltage 110VAC. This is approximately the RMS voltage on the AC power circuit. 
