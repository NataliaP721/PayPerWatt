...
public class BluetoothService {
    ...
    BluetoothService(Handler handler, BluetoothDevice device) {
    ...
    private void connectionFailed() {
        ...
        Message msg = myHandler.obtainMessage(Constants.MESSAGE_ERROR);
        Bundle bundle = new Bundle();
        bundle.putString(Constants.CONNECTION_ERROR, "Unable to connect");
        msg.setData(bundle);
    ...
    private void connectionLost() {
        ...
        Message msg = myHandler.obtainMessage(Constants.MESSAGE_ERROR);
        Bundle bundle = new Bundle();
        bundle.putString(Constants.CONNECTION_ERROR, "Connection was lost");
        msg.setData(bundle);
        myHandler.sendMessage(msg);
        setState(Constants.STATE_ERROR);
        cancelConnectedThread();
    ...
    private class ConnectThread extends Thread {
        ...
        public void run() {
            try {
                ...
                controlFlags.connectionFailed = true;
                try {
                    mmSocket.close();
        ...
        synchronized public void run() {
            ...
            while (true) {
                try {
                    ...
                    if (read.contains("\t")) {
                        //remove the tab - intentionally done twice in a row
                        readMessage.deleteCharAt(readMessage.length()-1);
                       // readMessage.deleteCharAt(readMessage.length()-1);
